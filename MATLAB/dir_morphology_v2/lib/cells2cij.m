    ...
    while foundCells < nAllCells      
        ...
        if ~isempty(csvFile{rowLooper,cellIdColNum})     
            ...
            if strcmpi(csvFile{rowLooper,axonOrDendriteColNum}, 'axons')
                ...
                bPos = strfind(csvFile(rowLooper,cellStatusColNum), 'B');
                aPos = strfind(csvFile(rowLooper,cellStatusColNum), 'A');
                ...
                uPos = strfind(csvFile(rowLooper,cellStatusColNum), 'U');
                rPos = strfind(csvFile(rowLooper,cellStatusColNum), 'R');
                ...
                if (~isIncludeV1p0Rank1to3ActiveTypes) && (~isempty(nPos{1}))
                    % do nothing;
                elseif (~isIncludeV1p0Rank4to5ActiveTypes) && (~isempty(mPos{1}))
                    % do nothing;
                elseif (~isIncludeV1p0ActiveTypesDisappearingInV2p0) && (~isempty(oPos{1}))
                    % do nothing;
                elseif (~isIncludeV1p0ActiveTypesWithApprovedActiveAddenda) && (~isempty(bPos{1}))
                    % do nothing;
                elseif (~isIncludeV1p0ActiveTypesWithUnapprovedActiveAddenda) && (~isempty(aPos{1}))
                    % do nothing;
                elseif (~isIncludeApprovedV2p0ActiveTypes) && (~isempty(pPos{1}))
                    % do nothing;
                elseif (~isIncludeUnapprovedV2p0ActiveTypes) && (~isempty(vPos{1}))
                    % do nothing;
                elseif (~isIncludeV1p0OnholdTypesThatAreNowUnapprovedV2p0ActiveTypes) && (~isempty(wPos{1}))
                    % do nothing;
                elseif (~isIncludeApprovedV2p0OnholdTypesThatAreNowUnapprovedv2p0Active) && (~isempty(uPos{1}))
                    % do nothing;
                elseif (~isIncludeUnapprovedV2p0ActiveThatWillSplitOffFromV1p0Active) && (~isempty(rPos{1}))
                    % do nothing;
                elseif (~isIncludeV1p0OnholdTypes) && (~isempty(xPos{1}))
                    % do nothing;
                elseif (~isIncludeV1p0OnholdTypesThatWillDisappearInV2p0) && (~isempty(tPos{1}))
                    % do nothing;
                elseif (~isIncludeApprovedV2p0OnholdTypes) && (~isempty(yPos{1}))
                    % do nothing;
                elseif (~isIncludeUnapprovedV2p0OnholdTypes) && (~isempty(sPos{1}))
                    % do nothing;                   
                else
