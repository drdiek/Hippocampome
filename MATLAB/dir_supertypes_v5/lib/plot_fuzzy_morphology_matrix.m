function plot_fuzzy_morphology_matrix(morphologyMatrix,subregionsMatrix,labelsMatrix,eoriMatrix,rawData,...
    ...
    [TF, indices] = cellfind('DG',subregionsMatrix);
    nCells(DG) = length(indices);
    [TF, indices] = cellfind('CA3',subregionsMatrix);
    nCells(CA3) = length(indices);
    [TF, indices] = cellfind('CA2',subregionsMatrix);
    nCells(CA2) = length(indices);
    [TF, indices] = cellfind('CA1',subregionsMatrix);
    nCells(CA1) = length(indices);
    [TF, indices] = cellfind('Sub',subregionsMatrix);
    nCells(SUB) = length(indices);
    [TF, indices] = cellfind('EC',subregionsMatrix);
...
%     end
    ...
    morphologyColormap = [ WHITE;      ... %  0: A = 0
                           WHITE;  ... %  1: A = 1
                           RED;        ... %  2: A = 2
                           RED;        ... %  3: A = 3
                           WHITE;      ... %  4: D = 0
...
%    axis equal
    ...
    for i = 1:nAllCells
        for j = 1:nParcels
            %%% add white hollows to neurite locations that subtract a neurite from a canonical location
            if ((morphologyInvasion(i,j) == 2) || (morphologyInvasion(i,j) == 6))
                rectangle('Position',[j+0.25, i+0.375, 0.5, 0.25], 'EdgeColor', WHITE, 'FaceColor', WHITE);
            end
            
            %%% add white thick borders to neurite locations that add a neurite to a canonical location
            if (morphologyInvasion(i,j) == 1)
                rectangle('Position',[j+0.25, i+0.375, 0.5, 0.25], 'linewidth', 0.125, 'EdgeColor', RED_LIGHT, 'FaceColor', RED_LIGHT);
            end    
            if (morphologyInvasion(i,j) == 5)
                rectangle('Position',[j+0.25, i+0.375, 0.5, 0.25], 'linewidth', 0.125, 'EdgeColor', BLUE_LIGHT, 'FaceColor', BLUE_LIGHT);
            end    
        end % j
    end %i
    ...
    for j = 1:nParcels+1
        line([j,j], [1,nAllCells+1], 'linewidth', 0.5, 'color', BLACK);
    end
    
    line_width = 1.5;

    vLine = [-2.75, nAllCells+1];
    ...
    %%% title
    if isPlotSupertypesOnly
        strng = sprintf('%d Supertypes %s', nAllCells/2, datestr(now, 'yyyymmdd'));
    elseif isPlotSuperfamiliesOnly
        strng = sprintf('%d Superfamilies %s', nAllCells/2, datestr(now, 'yyyymmdd'));
    else
        strng = sprintf('%d Types %s', nAllCells/2, datestr(now, 'yyyymmdd'));
    end
...
%         label_code_cat = sprintf('%s {%d}', label_code_cat, nSubGrouping(c));
    ...
    elseif isPlotSuperfamiliesOnly
        ...
        plotFileName = sprintf('./output/morphology_matrix_types');
    end
