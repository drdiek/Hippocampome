function plot_axon_dendrite_separate(csvFile)
...
%     DGparcellations = 101:103;
%     CA3parcellations = 201:205;
%     CA2parcellations = 301:304;
%     CA1parcellations = 401:404;
%     SUBparcellations = 501:503;
%     ECparcellations = 601:606;

%     nParcellations = [length(DGparcellations) length(CA3parcellations) length(CA2parcellations) ...
% 		      length(CA1parcellations) length(SUBparcellations) length(ECparcellations) ];

%     DGtab = 1 + nOfficialParcellations - nParcellations(DG)/2;
%     CA3tab = 1 + nOfficialParcellations - nParcellations(DG) - nParcellations(CA3)/2;
%     CA2tab = 1 + nOfficialParcellations - nParcellations(DG) - ...
% 	     nParcellations(CA3) - nParcellations(CA2)/2;
%     CA1tab = 1 + nOfficialParcellations - nParcellations(DG) - ...
% 	     nParcellations(CA3) - nParcellations(CA2) - nParcellations(CA1)/2;
%     SUBtab = 1 + nOfficialParcellations - nParcellations(DG) - ...
% 	     nParcellations(CA3) - nParcellations(CA2) - nParcellations(CA1) ...
% 	     - nParcellations(SUB)/2;
%     ECtab = 1 + nOfficialParcellations - nParcellations(DG) - ...
% 	     nParcellations(CA3) - nParcellations(CA2) - nParcellations(CA1) ...
% 	     - nParcellations(SUB) - nParcellations(EC)/2;
    ...
    if isJanelia
        ...
        text(-3, nOfficialParcellations+9.5, hippocampomeVersion, ...
    ...
    for i = 1:nAllCells
        ...
        elseif (cellEorI{i} == 'I')
            ...
            cellLabel = deblank(cellLabel);
            if isJanelia
                text(i+0.5, nOfficialParcellations+1.55, cellLabel, 'color', [0.7 0.7 0.7], 'HorizontalAlignment', 'right', 'Rotation', 90, 'FontSize', 6)
            else
                text(i+0.5, nOfficialParcellations+2.3, cellLabels{i}, 'color', [0.7 0.7 0.7], 'HorizontalAlignment', 'right', 'Rotation', 90, 'FontSize', 6)
            end
            if(notes(i) ~= -1)
                if isJanelia
    ...
    if (isunix)
        ...
        delete 'Cij_axon-dend.tif';
        print(gcf, '-dtiff', '-r800', './Cij_axon-dend.tif');
    end
