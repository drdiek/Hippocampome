function PT_neurites()
    ...
    nParcels = -5; % Do not count "Number of cells" and "Actual pattern" and "E OR I" and "Targeting" and "PC?"
    parcelNo = 1;
    for j = 1:nCols
        columnHeaderStr = cell2mat(txt(4,j));
        if ~isempty(columnHeaderStr)
            nParcels = nParcels + 1;
            if (strcmp(columnHeaderStr(end-1:end), 'SP') || strcmp(columnHeaderStr(end-1:end), 'SG'))
                isPrincipalCellLayer(parcelNo) = 1;
            else
                isPrincipalCellLayer(parcelNo) = 0;
            end
            parcelNo = parcelNo + 1;
        end % if
    end % j
    ...
    for i = 1:nNeurons
        neuronType{i} = txt(2+i*4,1);
    end % i
    
    for i = 1:nNeurons
        for j = 1:nParcels
            % read in percent of presence values for axons
            if ~isnan(num(-3+i*4,-1+j*2))
                percentOfPresenceAxons(i,j) = num(-3+i*4,-1+j*2);
            end
            % read in percent of presence values for dendrites
            if ~isnan(num(-3+i*4+2,-1+j*2))
                percentOfPresenceDendrites(i,j) = num(-3+i*4+2,-1+j*2);
            end
        end % j
        
        % read in Number of cells
        nCellsRecorded(i) = num(-3+i*4,13);
        
        % read in E or I value
        isExcitatory(i) = strcmp(txt(2+i*4,16), 'E');
    
        % read in Targeting value
        if strcmp(txt(2+i*4,17), 'AD')
            isAxonalDendriticTargeting(i) = 1;
        elseif strcmp(txt(2+i*4,17), 'IS')
            isInterneuronSpecificTargeting(i) = 1;
        elseif strcmp(txt(2+i*4,17), 'S')
            isSomaticTargeting(i) = 1;
        end
        
        % read in PC? value
        if strcmp(txt(2+i*4,18), 'Y')
            isPyramidalCell(i) = 1;
        else
            isPyramidalCell(i) = 0;
        end
    ...
    [num, txt] = xlsread(fileName, 'Density summary');
    
    for i = 1:nNeurons
        for j = 1:nParcels
            % read in density values for axons
            if ~isnan(num(-3+i*4,-1+j*2))
                densityAxons(i,j) = num(-3+i*4,-1+j*2);
            end
            % read in density values for dendrites
            if ~isnan(num(-3+i*4+2,-1+j*2))
                densityDendrites(i,j) = num(-3+i*4+2,-1+j*2);
            end
        end % j
...
%     connectionProbabilities = -ones(nNeurons,nNeurons,nParcels);
    for iA = 1:nNeurons
        for iD = 1:nNeurons
    ...
    minMin = min(min(min(connectionProbabilities(connectionProbabilities > 0))))
    
    connectionProbabilitiesMax = max(max(max(connectionProbabilities)))
    connectionProbabilitiesMax/minMin
    round(connectionProbabilitiesMax/minMin+0.5)
    pause
