function neurites()
    ...
    for i = 1:nNeurons
        ...
        range = sprintf('%c%d:%c%d', char(65+1*2), 2+i*4, char(65+nParcels*2), 2+i*4); % C6, E6, G6, ...; C10, E10, G10, ...; C14, E14, G14, ...; ...
        [num] = xlsread(fileName, '% of Presence', range)
        j = 0;
        for k = 1:length(num)
            if ~isempty(num(k))
                j = j + 1;
                percentOfPresenceAxons(i,j) = num(k)
            end % if
        end % k
        pause
        % read in percent of presence values for dendrites
        range = sprintf('%c%d:%c%d', char(65+1*2), 2+i*4+2, char(65+nParcels*2), 2+i*4+2); % C8, E8, G8, ...; C12, E12, G12, ...; C16, E16, G16, ...; ...
        [num] = xlsread(fileName, '% of Presence', range);
        for k = 1:length(num)
            if ~isempty(num(k))
                percentOfPresenceDendrites(i,j) = num(k);
            end % if
        end % k
