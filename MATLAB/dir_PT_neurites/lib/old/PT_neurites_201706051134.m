...
%     fileName = 'Quantification_of_Neurites_table_contacts_updated.xlsx';
    ...
    for i = 1:nRows
        ...
        if ~isempty(cellNameStr)
            nNeurons = nNeurons + 1;
            if (nNeurons > -1)
                neuronTypeNames{nNeurons+1} = txt(i,1);
            end
        end % if
    end % i
    ...
    nParcels = -6; % Do not count "Number of cells" and "Actual pattern" and "E OR I" and "Targeting" and "PC?" and "Soma in PCL?"
    parcelNo = 1;
    for j = 1:nCols
    ...
    for i = 1:nNeurons
        ...
        elseif strcmp(txt(2+i*4,17), 'AIS')
    ...
    connectionProbabilitiesScaled = round(connectionProbabilities*nProbabilityColors/connectionProbabilitiesMax + 0.5);
