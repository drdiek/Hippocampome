...
%
[properties, typeIds, nTypeIds, typeNames, propertyNames] = load_contingency_matrices_properties();
...
fid = fopen('contingency_matrices_values.csv', 'w');  % initialize file for saving pValues

fprintf(fid, 'A,B,A AND B,A AND notB,notA AND B,notA AND notB,Sum,Interesting?,Auto-Interpretation\n');  % print column headers to the file
...
for i = 1:nProperties
    ...
    for j = i+1:nProperties
        ...
        if (mod(isInteresting,1))
            ...
            fprintf(fid, '%s,%s,%d,%d,%d,%d,%d,%d,%s\n', propertyNames{i}, propertyNames{j}, a, b, c, d, n, isInteresting, autoInterpretationStr);
        
        else
            
            fprintf(fid, '%s,%s,%d,%d,%d,%d,%d,%.1f,%s\n', propertyNames{i}, propertyNames{j}, a, b, c, d, n, isInteresting, autoInterpretationStr);
