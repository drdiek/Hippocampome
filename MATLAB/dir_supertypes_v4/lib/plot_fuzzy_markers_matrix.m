function plot_fuzzy_markers_matrix(markersMatrix, rawData)
    ...
    mm = markersMatrix(1:15,:)
    
    idx = find(markersMatrix > 1);
    markersMatrix(idx) = markersMatrix(idx) + 1;
    
    idx = find(markersMatrix < 0);
    markersMatrix(idx) = 2;
    ...
    markersColormap = [  WHITE;        ... %  0: unknown
                         GREEN_BRIGHT; ... %  1: positive
                         BLUE;         ... %  2: negative
                         YELLOW; ... %  3: positive-negative (subtypes)
                         ORANGE;       ... %  4: positive-negative (species/protocol differences)
                         PURPLE;       ... %  5: positive-negative (subcellular expression differences)
                         RED];         ... %  6: positive-negative (unresolved)
    ...
    colormap(markersColormap(1:size(markersColormap,1),:));
%     colormap(markersColormap(1:3,:));
    
    markersExpression = zeros(nAllCells+1,nMarkers+1);

    markersExpression(1:nAllCells,1:nMarkers) = markersMatrix(1:nAllCells,1:nMarkers);
