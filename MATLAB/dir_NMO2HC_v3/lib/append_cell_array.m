function cellArray = append_cell_array(cellArray, dataStruct, brainRegion, cellType, nNeuronResources, nBrainRegions2Save, ...
    ...
    for i = 1:nNeuronResources
        ...
        for j = 1:nBrainRegions
            cellArray{i+nRows,j+c} = dataStruct.alpha__embedded.neuronResources{1,i}.brain_region{1,j};
        end
        c = 9+nBrainRegions2Save+3;
        nCellTypes = size(dataStruct.alpha__embedded.neuronResources{1,i}.cell_type,2);
        for j = 1:nCellTypes
            cellArray{i+nRows,j+c} = dataStruct.alpha__embedded.neuronResources{1,i}.cell_type{1,j};
        end
        ...
        for j = 1:nDois
            cellArray{i+nRows,j+c} = dataStruct.alpha__embedded.neuronResources{1,i}.reference_doi{1,j};
        end
        c = c+nDois2Save+1;
