function cellArray = cell_array_header(nBrainRegions2Save, nCellTypes2Save, nPmids2Save, nDois2Save)
    ...
    i = 9;
    for j = 1:nBrainRegions2Save
        cellArray{1,j+i} = sprintf('brain_region_%d', j);
    end
    i = 9+nBrainRegions2Save+1;
    cellArray{1,i} = 'Hippocampome_name';
    i = i + 1;
    cellArray{1,i} = 'Hippocampome_status';
    i = i + 1;
    cellArray{1,i} = 'match_flag';
    for j = 1:nCellTypes2Save
        cellArray{1,j+i} = sprintf('cell_type_%d', j);
    end
    i = i+nCellTypes2Save+1;
    ...
    i = i+nPmids2Save;
    for j = 1:nDois2Save
        cellArray{1,j+i} = sprintf('reference_doi_%d', j);
    end
    i = i+nDois2Save+1;
