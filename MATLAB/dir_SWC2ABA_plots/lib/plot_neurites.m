function plot_neurites(neurites, neuronLabels, parcelLabels)
...
%     end
    ...
    morphologyInvasion(1:nNeurons,1:nParcels) = morphologyMatrix(1:nNeurons,1:nParcels);
...
%    axis equal
    ...
    %%% title
    if isPlotSupertypesOnly
        strng = sprintf('%d Supertypes %s', nNeurons/2, datestr(now, 'yyyymmdd'));
    elseif isPlotSuperfamiliesOnly
        strng = sprintf('%d Superfamilies %s', nNeurons/2, datestr(now, 'yyyymmdd'));
    else
        strng = sprintf('%d Types %s', nNeurons/2, datestr(now, 'yyyymmdd'));
    end
