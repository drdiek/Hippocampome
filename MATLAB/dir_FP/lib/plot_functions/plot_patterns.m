function plot_patterns()
    ...
    nPatternOccurrences = zeros(nAllCells+1, nPatterns+1);

    nPatternOccurrences(1:nAllCells, 1:nPatterns) = nPatternOccurrencesSorted;
    
    pcolor(nPatternOccurrences)
    ...
    % add numbers to colord squares
    for i = 1:nAllCells
        for j = 1:nPatterns
            if (nPatternOccurrences(i,j) > 0)
                text(j+0.5, i+0.5, num2str(nPatternOccurrences(i,j)), 'Horizontalalignment', 'center', 'Rotation', 0, 'FontSize', displayFontSize, 'color', WHITE); 
            end
        end
    end
...
%     line([0,0], [1, nPatterns], 'linewidth', line_width, 'color', [0 0 0])
    line([-10, nPatterns+vStart+3.5], [DGline,  DGline],  'linewidth', line_width, 'color', BLACK)
    line([-10, nPatterns+vStart+3.5], [CA3line, CA3line], 'linewidth', line_width, 'color', BLACK)
    line([-10, nPatterns+vStart+3.5], [CA2line, CA2line], 'linewidth', line_width, 'color', BLACK)
    line([-10, nPatterns+vStart+3.5], [CA1line, CA1line], 'linewidth', line_width, 'color', BLACK)
    line([-10, nPatterns+vStart+3.5], [SUBline, SUBline], 'linewidth', line_width, 'color', BLACK)
    ...
    vTab = vTab + 0.5;
    hTab = hTab - 1.5;
    text(hTab, vTab, '1 occurrence of the pattern', 'HorizontalAlignment', 'left', ...
        'Rotation', 0, 'FontSize', displayFontSize, 'Color', ORANGE_LIGHT);
    
    vTab = vTab + 1.0;
    text(hTab, vTab, '2 occurrences of the pattern', 'HorizontalAlignment', 'left', ...
        'Rotation', 0, 'FontSize', displayFontSize, 'Color', BLUE);
    
    vTab = vTab + 1.0;
    text(hTab, vTab, '3 occurrences of the pattern', 'HorizontalAlignment', 'left', ...
        'Rotation', 0, 'FontSize', displayFontSize, 'Color', BROWN);
    
    vTab = vTab + 1.0;
    text(hTab, vTab, '4 occurrences of the pattern', 'HorizontalAlignment', 'left', ...
    ...
    plotFileName = sprintf('./output/FP_plot_output/FP_matrix');
